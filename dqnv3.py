...
class DQNAgentV2(DQNAgent):
    def __init__(
        ...
        self.running_estimators = {}
        self.reward_factor = reward_factor
        self.optimizer_intrinsic = torch.optim.Adam(self.predictor.parameters(), lr=learning_rate/10, weight_decay=weight_decay, amsgrad=amsgrad)
    ...
    def update(
        ...
        if len(self.running_estimators.keys())==0:
            with torch.no_grad():
                ...
                mse = intrinsic_loss.item()
            self.running_estimators["mse_mean"] = mse
            self.running_estimators["state_mean"] = obs
            self.running_estimators["count"] = 1
            self.running_estimators["state_std"] = (0,0)
            self.running_estimators["mse_std"] = 0
        else:
            if self.running_estimators["count"]<self.min_samples:
                with torch.no_grad():
                    predicted_intrinsic = self.predictor(torch.tensor(obs, device=self.device).unsqueeze(0))
                    target_intrinsic = self.target_predictor(torch.tensor(obs, device=self.device).unsqueeze(0))
                    intrinsic_loss = self.criterion_intrinsic(predicted_intrinsic, target_intrinsic)
                    mse = intrinsic_loss.item()
            else:
                normalized_obs = torch.tensor((obs-self.running_estimators["state_mean"])/self.running_estimators["state_std"], device=self.device, dtype=torch.float32).unsqueeze(0)
                predicted_intrinsic = self.predictor(normalized_obs)
                target_intrinsic = self.target_predictor(normalized_obs)
                intrinsic_loss = self.criterion_intrinsic(predicted_intrinsic, target_intrinsic)
                mse = intrinsic_loss.item()
                RND = self.reward_factor*np.clip((mse-self.running_estimators["mse_mean"])/self.running_estimators["mse_std"], -5,5)
                reward += RND
                if self.running_estimators["count"] >= self.replay_buffer.replay_size:
                    intrinsic_loss.backward()
                    self.optimizer_intrinsic.step()

            self.running_estimators["count"] += 1
            previous_mse_mean = self.running_estimators["mse_mean"]
            previous_state_mean = self.running_estimators["state_mean"]
            self.running_estimators["mse_mean"] += (mse-previous_mse_mean)/self.running_estimators["count"]
            self.running_estimators["state_mean"] += (obs-previous_state_mean)/self.running_estimators["count"]
            if self.running_estimators["count"]>=self.min_samples:
                self.running_estimators["state_std"] += (obs-previous_state_mean)*(obs-self.running_estimators["state_mean"])
                self.running_estimators["mse_std"] += (mse-previous_mse_mean)*(mse-self.running_estimators["mse_mean"])
